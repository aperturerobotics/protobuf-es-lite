...
describe('unixMilliToDate', () => {
  ...
  it('should throw an error for values outside the safe range of Number', () => {
    ...
    expect(() => unixMilliToDate(tooBig)).toThrowError("Timestamp is outside the range that can be safely represented by a JavaScript Date");
    expect(() => unixMilliToDate(tooSmall)).toThrowError("Timestamp is outside the range that can be safely represented by a JavaScript Date");
  });
});
