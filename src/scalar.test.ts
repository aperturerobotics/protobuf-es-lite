...
import { ScalarType, scalarEquals, scalarZeroValue, isScalarZeroValue, normalizeScalarValue, LongType } from './scalar';
...
describe('scalarEquals', () => {
  it('returns true for equal values', () => {
    ...
    expect(scalarEquals(ScalarType.BYTES, new Uint8Array([1, 2, 3]), new Uint8Array([1, 2, 3]))).toBe(true);
  });
  ...
  it('returns false for unequal values', () => {
    ...
    expect(scalarEquals(ScalarType.BYTES, new Uint8Array([1, 2, 3]), new Uint8Array([4, 5, 6]))).toBe(false);
  });
...
describe('scalarZeroValue', () => {
  it('returns zero value for each scalar type', () => {
    ...
    expect(scalarZeroValue(ScalarType.BYTES, LongType.BIGINT)).toEqual(new Uint8Array(0));
...
describe('isScalarZeroValue', () => {
  ...
  it('returns false for non-zero values', () => {
    ...
    expect(isScalarZeroValue(ScalarType.BYTES, new Uint8Array([1]))).toBe(false);
...
describe('normalizeScalarValue', () => {
  it('returns normalized value', () => {
    ...
    expect(normalizeScalarValue(ScalarType.STRING, 'hello', false)).toBe('hello');
    expect(normalizeScalarValue(ScalarType.BYTES, [1, 2, 3], false)).toEqual(new Uint8Array([1, 2, 3]));
