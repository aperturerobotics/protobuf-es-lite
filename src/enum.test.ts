...
import { createEnumType, enumInfoZeroValue, enumDescZeroValue, enumZeroValue, normalizeEnumValue, EnumValueInfo } from './enum.js';
...
describe('enum', () => {
  ...
  describe('createEnumType', () => {
    ...
    it('should find enum values by name', () => {
      expect(TestEnum.findName('ZERO')).toEqual({ no: 0, name: 'ZERO', localName: 'ZERO' });
      expect(TestEnum.findName('ONE')).toEqual({ no: 1, name: 'ONE', localName: 'ONE' });
      expect(TestEnum.findName('TWO')).toEqual({ no: 2, name: 'TWO', localName: 'TWO' });
      expect(TestEnum.findName('THREE')).toBeUndefined();
    });

    it('should find enum values by number', () => {
      expect(TestEnum.findNumber(0)).toEqual({ no: 0, name: 'ZERO', localName: 'ZERO' });
      expect(TestEnum.findNumber(1)).toEqual({ no: 1, name: 'ONE', localName: 'ONE' });
      expect(TestEnum.findNumber(2)).toEqual({ no: 2, name: 'TWO', localName: 'TWO' });
  ...
  describe('enumDescZeroValue', () => {
    it('should throw for an empty enum', () => {
      expect(() => enumDescZeroValue({ values: [] } as any)).toThrow('invalid enum: missing at least one value');
    });
  ...
  describe('enumZeroValue', () => {
    it('should throw for an empty enum', () => {
      expect(() => enumZeroValue({ values: [] } as any)).toThrow('invalid enum: missing at least one value');
    });
  ...
  describe('normalizeEnumValue', () => {
    ...
    it('should throw for an invalid name', () => {
      expect(() => normalizeEnumValue(TestEnum, 'THREE')).toThrow('enum TestEnum: invalid value: "THREE"');
    });
